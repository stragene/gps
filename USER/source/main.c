...
int main(void)
{
    ...
    xTaskCreate((void *)vTaskRsvPPP, "RsvPPP", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1, NULL);
...
void vTaskRsvPPP(void)
{
    ...
    for(;;)
    {
        xSemaphoreTake(xSemGprsRsvd, portMAX_DELAY);
        len = Uart_IdleRead(pUartGPRS, pbuf, BUF_SIZE, 0);
        pppos_input_tcpip(pppGprs, pbuf, len);
    }
...
{
   uint16_t holdoff;
   pppGprs = pppos_create(&pppGprs_netif, output_cb, status_cb, NULL);
